function [SM, EN, SYr, SXr, SE, SG, OPP]=Extrema_Single_SpiralTraverse(img)
% Created by Anton and Tsvetkov, BSUIR, 2019: Scanline-spiral, Tuan Pham [13]
...
while y<Y
    ...
    while x<X
        N=N+11;
        % Scanning Mask: [5 6 7; 4 0 8; 3 2 1]
        S5=img(y-1,x-1); S6=img(y-1,x); S7=img(y-1,x+1);
        S4=img(y,x-1);   S0=img(y,x);   S8=img(y,x+1);
        S3=img(y+1,x-1); S2=img(y+1,x); S1=img(y+1,x+1);               
        % 1.Find Local Mixima        
        if BH(y,x)==1
            %N=N+1;
            if S0>S1
